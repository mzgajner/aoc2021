...
const findPathsRecursion = (
  ...
  if (current == goal) {
    ...
    const relevantNeighbours = current.edges.map((e) => e.t).filter((v) =>
      !v.isSmallCave() || !currentPath.includes(`(${v.key})`)
    );
    relevantNeighbours.forEach((vertex) => {
      findPathsRecursion(vertex, goal, currentPath, pathsFound);
    });
  }
};
